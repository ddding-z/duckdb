...
R1_hotels.to_csv(outpath1 + path2, index=False)
R2_searches.to_csv(outpath1 + path3, index=False)

R1_hotels.to_csv(outpath2 + path2, index=False)
R2_searches.to_csv(outpath2 + path3, index=False)

X = S_listings.drop("promotion_flag", axis=1)
