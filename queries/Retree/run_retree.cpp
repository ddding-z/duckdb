...
std::string PATH = "../queries/";
std::string SQL_PATH = "./workloads/";
std::string MODEL_PATH = "../../workloads/";
...
void run(const Config &config) {
	...
	if (config.optimization_level == 1) {
		con.Query(read_file( + "load_convert_rule.sql"));
	...
	std::vector<std::string> predicates = read_predicates(MODEL_PATH +"/predicates.txt");
	std::ofstream outputfile(sql_path + "/output.csv", std::ios::app);

	std::vector<double> records;

	std::string set = replacePlaceholder("set threads = ?;", "?", config.thread);
	con.Query(set);
	if (config.model_type == "dt"){
		con.Query(replacePlaceholder(read_file(sql_path + "/load_data_dt.sql"), "?", config.scale));
	} else {
