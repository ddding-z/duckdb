...
//** rule1: 剪枝
class DTPruneExtension : public OptimizerExtension {
public:
	DTPruneExtension() {
		optimize_function = pruneDTRule;
	}
...
public:
	...
	static void pruneDTRule(OptimizerExtensionInput &input, duckdb::unique_ptr<LogicalOperator> &plan) {
...
//===--------------------------------------------------------------------===//
extern "C" {
DUCKDB_EXTENSION_API void retree_prune_rule_extension_init(duckdb::DatabaseInstance &db) {
	Connection con(db);
	auto &config = DBConfig::GetConfig(db);

	// add a parser extension: 剪枝
	config.optimizer_extensions.push_back(DTPruneExtension());
	config.AddExtensionOption("pruning", "onnx model pruning", LogicalType::INVALID);

}

DUCKDB_EXTENSION_API const char *retree_prune_rule_extension_version() {
