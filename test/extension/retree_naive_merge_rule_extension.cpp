...
//** rule2: 合并
class DTNavieMergeExtension : public OptimizerExtension {
public:
    DTNavieMergeExtension() {
		optimize_function = naiveMergeDTRule;
	}
...
public:
			...
			if (comparison_expr.left->expression_class == ExpressionClass::BOUND_FUNCTION) {
				...
				if (func_expr.function.name == "predict") {
					...
					if (original_model_path.find("pruned") != std::string::npos) {
						...
						auto opted_model_path =
						optimize_on_decision_tree_predicate_naive_merge(original_model_path, threads_count);
	...
	static void naiveMergeDTRule(OptimizerExtensionInput &input, duckdb::unique_ptr<LogicalOperator> &plan) {
...
//===--------------------------------------------------------------------===//
extern "C" {
DUCKDB_EXTENSION_API void retree_naive_merge_rule_extension_init(duckdb::DatabaseInstance &db) {
	Connection con(db);
	auto &config = DBConfig::GetConfig(db);

	// add a parser extension: 合并
	config.optimizer_extensions.push_back(DTNavieMergeExtension());
	config.AddExtensionOption("merge", "onnx model branch merging", LogicalType::INVALID);
}

DUCKDB_EXTENSION_API const char *retree_naive_merge_rule_extension_version() {
