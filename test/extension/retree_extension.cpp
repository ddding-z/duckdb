...
//** rules
class ReTreeExtension : public OptimizerExtension {
public:
    ReTreeExtension() {
		optimize_function = retreeRule;
	}
...
public:
			...
			if (comparison_expr.left->expression_class == ExpressionClass::BOUND_FUNCTION) {
				...
				if (func_expr.function.name == "predict") {
					...
					if (comparison_expr.right->type == ExpressionType::VALUE_CONSTANT) {
						...
						auto opted_model_path = optimize_on_decision_tree_predicate(
						    original_model_path, comparison_operator_, predicate, threads_count);
						// auto opted_model_path = optimize_on_decision_tree_predicate_prune(
						//     original_model_path, comparison_operator_, predicate, threads_count);
	...
	static void retreeRule(OptimizerExtensionInput &input, duckdb::unique_ptr<LogicalOperator> &plan) {
...
//===--------------------------------------------------------------------===//
extern "C" {
DUCKDB_EXTENSION_API void retree_extension_init(duckdb::DatabaseInstance &db) {
	Connection con(db);
	auto &config = DBConfig::GetConfig(db);
	
    // add a parser extension: Retree rules: convert, prune, merge
	config.optimizer_extensions.push_back(ReTreeExtension());
	config.AddExtensionOption("ReTree Optimization", "convert, prune, merge", LogicalType::INVALID);

}

DUCKDB_EXTENSION_API const char *retree_extension_version() {
